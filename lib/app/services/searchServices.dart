...
class SearchServices {
  ...
  static setHistoryData(keywords) async {
    ...
    if (searchListData != null) {
      bool hasData = searchListData.any((element) => element == keywords);
      if (hasData) {
      ...
      Storage.setData('searchList', searchListData);
    } else {
      List tempList = [];
...
//获取历史搜索记录
  ...
  ///删除数据
  static deleteHistoryData(String keywords) async {
    List searchListData = await Storage.getData('searchList');
  ...
  static clearHistoryData() async {
    await Storage.deleteData('searchList');
  }
}
